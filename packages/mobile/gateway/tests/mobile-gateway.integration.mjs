...
test('paired devices receive and answer only current DSH interactions', async (t) => {
  ...
  const unauthorized = await fetch(`${status.url}/v1/sessions/list`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: '{}' })
  assert.equal(unauthorized.status, 401)
  const authorized = await fetch(`${status.url}/v1/sessions/list`, { method: 'POST', headers, body: '{}' })
  assert.equal(authorized.status, 200)
  assert.deepEqual((await authorized.json()).data, { items: [{ sessionId: 'session-1', title: 'Fixture session' }] })
  ...
  stream.write(`data: ${JSON.stringify({
    ...
    method: 'events.mux',
    payload: { type: 'approval/requested', sessionId: 'session-1', approvalId: 'approval-1', toolName: 'shell', reason: 'Run a safe fixture command.' },
  })}\n\n`)
  ...
  const interactions = await eventually(async () => {
    ...
    if (!response.ok || payload.data === undefined) throw new Error('interaction endpoint returned ' + response.status + ': ' + JSON.stringify(payload))
  ...
  const response = await fetch(`${status.url}/v1/interactions/respond`, {
    method: 'POST',
    headers,
    body: JSON.stringify({ rpcId: 'approval-rpc-1', result: { ok: true, value: { sessionId: 'session-1', approvalId: 'approval-1', outcome: 'allowed-once' } } }),
  })
  ...
  const cleared = await fetch(`${status.url}/v1/sessions/session-1/interactions`, { method: 'POST', headers, body: '{}' })
