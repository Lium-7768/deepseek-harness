...
Each call validates positional inputs, constructs the descriptor's exact named `args`, and sends it through `ctx.connection.rpc.call('/api', endpoint, ...)`. When one method has both direct and scoped projections, an explicit direct form wins only when its strict business arguments match; otherwise a caller with the required Context can use the scoped alias. Generated cancellation-aware methods accept a final optional browser-native `AbortSignal`; other trailing values remain business-input validation errors and are never merged as cancellation. The Client combines a valid caller signal with the contribution mount lifetime before calling Connection. The returned value is validated before reaching application code. Withdrawing a contribution removes its descriptors and methods together, aborts in-flight calls, and makes retained method handles reject.
