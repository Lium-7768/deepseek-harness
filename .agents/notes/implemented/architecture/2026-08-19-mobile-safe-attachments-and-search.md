...
Android Debug now overrides React Native's development-server and inspector ports to 8090, matching the existing project Metro service without affecting the Release variant or another local service on 8081. Android 16 Debug APK assembly and streamed AVD installation pass. The Android 16 emulator subsequently entered a System UI ANR both before and after a clean reboot, so no claim is made for a completed Android visual runtime assertion; a healthy AVD or physical Android device must repeat that one check.
