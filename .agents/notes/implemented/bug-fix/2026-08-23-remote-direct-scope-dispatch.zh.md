...
API Gateway 客户端测试覆盖 agent 作用域中的显式直接调用。API Remote 矩阵会盘点 API Remote 聚合选择的每个贡献；任何 scope 或 cancellation 组合变化都会使测试失败。它装载命令、Goals、文件引用和会话引用，证明它们均保留显式直接 wire 字段；文件和会话引用还保留真实取消信号。包 README 记录了 direct/scoped 选择与取消规则。
