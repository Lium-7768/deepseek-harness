...
The API Gateway client test covers an explicit direct call from an agent-scoped context. It proves that the agent identity and request preserve their direct wire fields instead of being shifted into the scoped alias. The package README records the direct/scoped selection and cancellation rule.
